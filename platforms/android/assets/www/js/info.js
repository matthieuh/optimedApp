...
function setParams() {
	...
	syncArgs = "info"+"/"+device.platform+"/"+device.version;
	createTableSQL =	"CREATE TABLE IF NOT EXISTS info ( " +
...
function SQLUpdate(data,tx) {
	var sql =	"INSERT OR REPLACE INTO info (id, title, content, lastmodified) " +
				"VALUES (?, ?, ?, ?)";
...
function getContent(data) {
	...
	var content=data[0].content + data[1].content;
