...
function setParams() {
	...
	syncArgs = "legalnotice"+"/"+device.platform+"/"+device.version;
	createTableSQL =	"CREATE TABLE IF NOT EXISTS legalnotice ( " +
...
function SQLUpdate(data,tx) {
	var sql =	"INSERT OR REPLACE INTO legalnotice (id, title, content, lastmodified) " +
				"VALUES (?, ?, ?, ?)";
